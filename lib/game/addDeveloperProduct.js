...
exports.optional = ['description', 'iconImageAssetId', 'jar']

const nextFunction = (jar, xcsrf, universeId, prodName, priceInRobux, prodDescription, iconImageAssetId) => {
  return http({
    url: `//develop.roblox.com/v1/universes/${universeId}/developerproducts?name=${prodName}&description=${prodDescription}&priceInRobux=${priceInRobux}&iconImageAssetId=${iconImageAssetId}`,
    options: {
...
exports.func = (args) => {
  const jar = args.jar
  const prodDescription = args.description || ''
